...
constexpr uint64_t COMPLEMENT_MASK[] = {
...
struct TwoPointLinearReference {
  ...
  midgard::PointLL getFirstCoordinate() const {
    return midgard::PointLL(fixed2deg(bytes2fixed(fixed_longitude, 3)),
  ...
  unsigned getFirstFOW() const {
    return first_fow;
  }
  void setFirstFOW(unsigned fow_) {
    first_fow = fow_;
  }
  unsigned getFirstFRC() const {
    return first_frc;
  }
  void setFirstFRC(unsigned frc_) {
    first_frc = frc_;
  }

  unsigned getLastFOW() const {
    return last_fow;
  }
  void setLastFOW(unsigned fow_) {
    last_fow = fow_;
  }
  unsigned getLastFRC() const {
    return last_frc;
  }
  void setLastFRC(unsigned frc_) {
    last_frc = frc_;
  }

  void setFirstBearing(double bearing_) {
    first_bearing = static_cast<uint8_t>(std::floor(bearing_ / 11.25));
  }
  double getFirstBearing() const {
    return first_bearing * 11.25 + 11.25 / 2;
  }

  void setLastBearing(double bearing_) {
    last_bearing = static_cast<uint8_t>(std::floor(bearing_ / 11.25));
  }
  double getLastBearing() const {
    return last_bearing * 11.25 + 11.25 / 2;
  }

  void setLength(double length) {
    dist_to_next = length / 58.6;
  }
  double getLength() const {
    return dist_to_next * 58.6;
  }
  ...
  static TwoPointLinearReference fromBase64(const std::string& base64) {
    ...
    TwoPointLinearReference* tmp =
        reinterpret_cast<TwoPointLinearReference*>(const_cast<char*>(binary.data()));
